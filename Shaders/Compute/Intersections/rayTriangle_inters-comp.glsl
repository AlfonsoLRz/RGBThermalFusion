...
// array.
bool rayTriangleIntersection(uint index, in FaceGPUData face, in RayGPUData ray)
{
	...
	const float meshOpacity = meshData[face.modelCompID].opacity;

	if (meshOpacity < rand(v1.position.xy))
	{
		return false;
	}
	...
	vec3 intersPoint = ray.orig + ray.dir * t;
	float dist = distance(ray.startingPoint.xyz, intersPoint);
	float maxDist = maximumRange + (rand(intersPoint.xy) * 2.0f - 1.0f) / TIME_BOUNDARY_CONST;

	if (t > EPSILON && dist < maxDist)
	{
		collision[index].intersPoint = intersPoint;
		collision[index].intersNormal = normalize(face.normal);
		collision[index].intersTextCoord = getTextCoord(intersPoint, v1, v2, v3);
		collision[index].distance = t;				
		collision[index].angle = dot(normalize(face.normal), normalize(-ray.dir));
